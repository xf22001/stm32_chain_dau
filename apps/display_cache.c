 ...
 *   修改日期：2021年12月22日 星期三 15时00分40秒
 *   描    述：
 *
 *================================================================*/
...
{
	switch(channels_info->channels_settings.power_module_type) {
	...
	if(channels_info->display_cache_channels.power_module_type_sync != 0) {
		...
		switch(channels_info->display_cache_channels.power_module_type) {
			...
			case DISPLAY_POWER_MODULE_TYPE_HUAWEI: {
				channels_info->channels_settings.power_module_type = POWER_MODULE_TYPE_HUAWEI;
			}
			break;

			case DISPLAY_POWER_MODULE_TYPE_INCREASE: {
				channels_info->channels_settings.power_module_type = POWER_MODULE_TYPE_INCREASE;
			}
			break;

			case DISPLAY_POWER_MODULE_TYPE_INFY: {
				channels_info->channels_settings.power_module_type = POWER_MODULE_TYPE_INFY;
			}
			break;

			case DISPLAY_POWER_MODULE_TYPE_STATEGRID: {
				channels_info->channels_settings.power_module_type = POWER_MODULE_TYPE_STATEGRID;
			}
			break;

			case DISPLAY_POWER_MODULE_TYPE_YYLN: {
				channels_info->channels_settings.power_module_type = POWER_MODULE_TYPE_YYLN;
			}
			break;

			case DISPLAY_POWER_MODULE_TYPE_WINLINE: {
				channels_info->channels_settings.power_module_type = POWER_MODULE_TYPE_WINLINE;
			}
			break;

			case DISPLAY_POWER_MODULE_TYPE_ZTE: {
				channels_info->channels_settings.power_module_type = POWER_MODULE_TYPE_ZTE;
			}
			break;
